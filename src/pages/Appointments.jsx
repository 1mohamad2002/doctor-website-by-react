...
while(currentDate < endTime){
  ...
  // we added time 30 mins after the user time 
          ...
          <div className=' flex gap-3 items-center w-full overflow-scroll mt-4 '>

              {
                ...
                {docSlots.length && docSlots[slotIndex].map(
                  (item,index)=>(
                    <p  onClick={()=>setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0  px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-primary text-black':'text-gray-400 border-gray-300'}`} key={index}>
